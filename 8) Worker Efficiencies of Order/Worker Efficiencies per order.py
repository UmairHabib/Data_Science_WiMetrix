...
def timeRangeDataGetter(strInitial, strFinal,orderno):
 ...
 order by p_date '''
  ...
  AllBundle = list(df['bundleID'])
  return AllBundle
def worker_Matrix_Data_Getter(orderno):
   query1 = '''SELECT  [time]
         ,[bundleID]
         ,[operationAutoID]
         ,[macID]
...
def differenceColumn(df):
   ...
   arr.append(row['SAM'])
   return arr
def MedianTaker(workerID,allOperationAutoID , df2):
   arr=[np.nan]* len(allOperationAutoID)
   arr2=[np.nan]* len(allOperationAutoID)
   for index,each in enumerate(allOperationAutoID):
       x=df2[df2['operationAutoID']==each]
       if(len(x)!=0):
           arr[index]=sum(x['timeDifference'])
           arr2[index]=sum(x['SAM'])
   return arr,arr2

def workerMatrix(orderno):
   ...
   df2['timeDifference']=differenceColumn(df2)
   actualWorkerSam = pd.DataFrame(columns=allOperationAutoID, index=df2['workerID'].unique())
   idealWorkerSam = pd.DataFrame(columns=allOperationAutoID, index=df2['workerID'].unique())
   for workerID,row in  actualWorkerSam.iterrows():
       val,val2=MedianTaker(workerID,allOperationAutoID , df2[df2['workerID']==workerID])
       actualWorkerSam.loc[workerID] = val
       idealWorkerSam.loc[workerID]=val2

   # print(actualWorkerSam)
   # print(idealWorkerSam)
   differenceWorkerSam=idealWorkerSam.subtract(actualWorkerSam)
   print(differenceWorkerSam)
   # matrixFrame.to_csv(r"E:\data.csv")
   # bundles=16
   # for i in range(bundles):
   #     colors.append('%06X'% randint(0,0xFFFFFF))
   # counter=0
